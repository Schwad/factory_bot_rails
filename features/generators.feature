Feature:
  ...
  I would like to use factory_face_rails generators.
  ...
  Scenario: The factory_face_rails generators create a factory file for each model if there is not a factories.rb file
  ...
  Scenario: The factory_face_rails generators add a factory in the correct spot
    When I run `bundle install` with a clean environment
  ...
  Scenario: The factory_face_rails generators does not create a factory file for each model if there is a factories.rb file in the test directory
    When I run `bundle install` with a clean environment
    And I write to "test/factories.rb" with:
      """
      FactoryBot.define do
